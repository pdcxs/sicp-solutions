...
; and the answer is given by the value of $a$ at the end
; of the process. In general, the technique of defining
; an invariant quantity that remains unchanged from state
; to state is a powerful way to think about the design
...
(for ([i (in-range 1 10)])
  (for ([j (in-range 1 10)])
    (check-eq? (expt i j) (fast-expt i j))))